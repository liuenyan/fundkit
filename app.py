...
def make_charts(nav_df, detail, fund_code, fund_name, stop_profit_on):
    ...
    ax1 = axes[0]
    ax1.plot(nav_df["date"], nav_df["unit_nav"], color="steelblue",
             lw=1.2, alpha=0.9, label="单位净值")
    ax1.plot(nav_df["date"], nav_df["acc_nav"], color="steelblue",
             lw=0.8, ls="--", alpha=0.6, label="累计净值")
    if not detail.empty:
        avg = (detail["total_cost"] / detail["total_units"]).replace(
...
with st.sidebar:
    ...
    freq = st.selectbox("定投频率", ["monthly", "weekly", "biweekly", "daily"],
                        format_func=lambda x: {"monthly": "每月",
                                                "weekly": "每周",
                                                "biweekly": "每两周",
                                                "daily": "每日"}[x])
    day = st.slider("每月定投日 (1-28)", 1, 28, 10, disabled=freq != "monthly")
    weekday = st.selectbox("每周定投日", [1, 2, 3, 4, 5],
                           format_func=lambda x: {1: "周一", 2: "周二",
                                                   3: "周三", 4: "周四",
                                                   5: "周五"}[x],
    ...
    st.subheader("费率")
    fee = st.number_input("申购费率 (%)", 0.0, 10.0, 0.15, 0.05, format="%.2f",
                          help="默认 0.15%")
    ...
    if strategy == "策略A: 目标止盈":
        c1, c2 = st.columns(2)
        with c1:
            take_profit = st.number_input("目标止盈收益率 (%)", 1, None, 20, 5,
                                          format="%d",
                                          help="收益达此百分比即卖出")
        with c2:
            tp_cycle = st.checkbox("循环止盈模式", True,
                                   help="止盈后重新开始定投")
    elif strategy == "策略B: 停投持有+移动止盈":
        c1, c2 = st.columns(2)
        with c1:
            stop_invest = st.number_input("停投触发收益率 (%)", 1, None, 20, 5,
                                          format="%d",
                                          help="收益达此百分比后停止定投，继续持有")
        with c2:
            trailing_stop = st.number_input("移动止盈回撤阈值 (%)", 1, 50, 8, 1,
                                            format="%d",
                                            help="从高点回撤此百分比即卖出")
        tp_cycle = st.checkbox("循环止盈模式", True,
                               help="止盈后重新开始定投")
...
with st.spinner("正在获取数据并计算…"):
    ...
    invest_dates = generate_dca_dates(
        nav_df, freq, start_str, end_str, day, weekday
    )
    ...
    detail, events, redeem_fee, final_val = simulate_dca(
        nav_df, invest_dates, amount, fee / 100,
    ...
    ann_ret = calc_annualized(
        total_ret, pd.Timestamp(start_str), pd.Timestamp(end_str))
...
if lumpsum:
    ...
    winner = "定投胜 🏆" if diff > 0 else (
        "一次性投入胜 🏆" if diff < 0 else "持平")
...
if events:
    with st.expander(f"📋 止盈事件（共 {len(events)} 次）", expanded=True):
        ...
        ev_df = ev_df.rename(columns={
            "date": "日期", "nav": "净值",
            "return_rate": "收益率(%)", "round_cost": "轮次成本",
            "profit": "盈利", "redeem_fee": "赎回费",
            "net_proceeds": "净到账", "reason": "原因",
        })
...
with st.expander("📋 交易明细", expanded=False):
    if stop_profit_on:
        ...
        display = detail[(detail["investment"] > 0)
                         | detail["date"].isin(event_dates)].copy()
    else:
        display = detail.copy()

    out = display.rename(columns={
        "date": "日期", "nav": "净值", "investment": "投入",
        "units_added": "新增份额", "total_units": "累计份额",
        "market_value": "市值", "return_rate": "收益率(%)",
    }).copy()
...
st.download_button("📥 导出 CSV", data=csv,
                   file_name=f"{fund_code}_dca_backtest.csv",
                   mime="text/csv")
