...
|------|------|------|
| `--fund` | 是 | 基金代码（6 位数字） |
| `--amount` | 条件① | 每期定投金额（元），`--value-avg` 启用时忽略 |
...
| `--value-avg` | 条件② | **价值平均** 每期市值增长目标（如 `1000`，取代 `--amount`） |
| `--va-max-multiple` | 否 | **价值平均** 每期最大投入倍数（默认 4.0） |
| `--va-min-amount` | 否 | **价值平均** 最低申购金额，元（默认 10.0） |
| `--output` | 否 | CSV 导出路径 |
| `--chart` | 否 | 图表输出目录（默认 `./charts`） |

> ① 仅当 `--value-avg` 未指定或等于 0 时必需。
> ② 指定 `--value-avg` 后 `--amount` 被忽略，定投金额由价值平均算法动态计算。
...
**价值平均**：每月市值增长 1000 元，最大投入不超过目标 4 倍，最低 10 元：

```bash
python -m backend.dca_backtest --fund 163415 --value-avg 1000 \
  --va-max-multiple 4 --va-min-amount 10 --start 2018-01-01
```

> 价值平均策略下，每期买入金额 = max(目标市值增长 - 当前市值, `--va-min-amount`)，且不超过 `目标市值增长 × va-max-multiple`。市场下跌时自动多买，上涨时少买甚至不买。
