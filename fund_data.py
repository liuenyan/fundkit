...
def enrich_fee_scale(result, scale_source=None, progress_placeholder=None):
    ...
    # 兜底: 仍缺失净值的基金（如新成立FOF不在rank API中）从 fund_open_fund_info_em 逐只获取
    nav_still_missing = result["单位净值"].isna()
    if nav_still_missing.any():
        for idx in result.index[nav_still_missing]:
            code = result.at[idx, "基金代码"]
            try:
                nav_df = ak.fund_open_fund_info_em(symbol=code, indicator="单位净值走势")
                if not nav_df.empty:
                    row = nav_df.iloc[-1]
                    nav_c = [c for c in nav_df.columns if "单位净值" in c]
                    if nav_c:
                        result.at[idx, "单位净值"] = float(row[nav_c[0]])
                    date_c = [c for c in nav_df.columns if "日期" in c or "date" in c.lower()]
                    if date_c:
                        result.at[idx, "净值日期"] = str(row[date_c[0]])
            except Exception:
                pass
